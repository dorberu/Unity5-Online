...
public class gameManage : MonoBehaviour {
	...
	void OnPhotonPlayerConnected(PhotonPlayer newPlayer){
		...
		if(PhotonNetwork.isMasterClient){
			...
			apLv = 
				Mathf.RoundToInt(apLv / PhotonNetwork.room.playerCount);
			apWp = 
				Mathf.RoundToInt(apWp / PhotonNetwork.room.playerCount);
			...
			foreach(RoomInfo game in PhotonNetwork.GetRoomList()){
				...
				if(rWp == variableManage.myWP){
					...
					if(rLv == variableManage.currentLv){
						...
						int tmpLv = 
							Mathf.Abs(variableManage.currentLv - rLv);
						if(bLv > tmpLv){
							bLv = tmpLv;
							roomName = game.name;
						}
					...
					//勝敗スコアが異なる
					int tmpWp = 
						Mathf.Abs(variableManage.myWP - rWp);
					if(bWp > tmpWp){
						bWp = tmpWp;
						roomName = game.name;
					}
